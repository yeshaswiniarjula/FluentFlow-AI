        ...
        try:
            async with httpx.AsyncClient(timeout=15.0) as client:
                ...
                if response.status_code == 200:
                    ...
                    if retries > 0:
                        logger.warning("Grok LLM rate limited (429). Retrying after 1 second...")
                        await asyncio.sleep(1.0)
                        return await self._generate_with_retry(messages, max_tokens, retries=retries - 1)
                    else:
                        logger.error("Grok LLM rate limited. Retries exhausted.")
                        return LLMResponse(latency_ms=latency_ms, success=False, error_message="Rate limit exceeded")
                elif response.status_code >= 500:
                    if retries > 0:
                        logger.warning(f"Grok LLM server error ({response.status_code}). Retrying after 1 second...")
                        await asyncio.sleep(1.0)
                        return await self._generate_with_retry(messages, max_tokens, retries=retries - 1)
                    else:
                        logger.error(f"Grok LLM server error: {response.status_code} - {response.text}")
                        return LLMResponse(latency_ms=latency_ms, success=False, error_message=f"Server error {response.status_code}")
                else:
                    logger.error(f"Grok LLM client error ({response.status_code}): {response.text}")
                    return LLMResponse(latency_ms=latency_ms, success=False, error_message=f"API Error {response.status_code}")

        except httpx.TimeoutException:
            latency_ms = int((time.time() - start_time) * 1000)
            logger.warning(f"LLM call timed out after {latency_ms} ms")
